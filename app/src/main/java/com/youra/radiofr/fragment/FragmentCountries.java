...
import com.youra.radiofr.adapter.AdapterCountries;
import com.youra.radiofr.asyncTask.LoadCountries;
import com.youra.radiofr.callback.Callback;
import com.youra.radiofr.interfaces.CountriesListener;
...
public class FragmentCountries extends Fragment {
    ...
    private AdapterCountries adapter;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        helper = new Helper(getActivity(), (position, type) -> {
            ...
            intent.putExtra("page_type", getString(R.string.countries));
    ...
    private void getData() {
        if (helper.isNetworkAvailable()) {
            LoadCountries loadCategory = new LoadCountries(new CountriesListener() {
                @Override
                public void onStart() {
                    if (arrayList.size() == 0) {
                ...
                @Override
                public void onEnd(String success, String verifyStatus, String message, ArrayList<ItemCountries> arrayListCountries) {
                    if (getActivity() != null) {
                        if (success.equals("1")) {
                            if (!verifyStatus.equals("-1")) {
                                if (arrayListCountries.size() == 0) {
                                    isOver = true;
                                    try {
                                        adapter.hideHeader();
                                    } catch (Exception e) {
                                    ...
                                    for (int i = 0; i < arrayListCountries.size(); i++) {
                                        arrayList.add(arrayListCountries.get(i));
                                        if (Callback.isNativeAd) {
            ...
            }, helper.callAPI(Callback.METHOD_COU, page, "", "", "", "", "", "", "", "", "", "", "", "", null));
    ...
    public void setAdapter() {
        if(!isScroll) {
            adapter = new AdapterCountries(getActivity(),  arrayList, (itemCountries, position) -> helper.showInterAd(position, ""));
